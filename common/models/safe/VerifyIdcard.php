...
use common\models\datamodels\Idcard;

class VerifyIdcard extends Idcard
{
     ...
     * 验证身份证格式是否正确
     */
    public static function verifyFormat($card_id)
    {
        if (!$card_id) {
            return ['status'=>false,'msg'=>'身份证号不能为空','code'=>11];
        }
        if(strlen($card_id)==18){//18位身份号码检测
            ...
            if(empty($check_area['province'] || empty($check['area']))){
                return ['status'=>false,'msg'=>'身份证地区非法','code'=>15];
            }
            ...
            if (preg_match($ereg,$card_id)) {  //测试出生日期的合法性
                ...
                if($last_check_code[$Y]==$check){
                    if(!Idcard::find()->where(['card'=>$card_id])->exists()){
                        $card_m=new Idcard();
                        $card_m->card=$card_id;
                        $card_m->status=2;
                        $card_m->add_time=time();
                        if(!$card_m->insert()){
                            //记录日志
                            print_r($card_m->getFirstErrors());
                        }
                    }
                    return ['status'=>true,'msg'=>'验证通过','code'=>10];
                }else{
                    return ['status'=>false,'msg'=>'身份证号码校验错误','code'=>14];
                }
            }else{
                return ['status'=>false,'msg'=>'身份证号码出生日期超出范围或含有非法字符','code'=>13];
            }
        }
        return ['status'=>false,'msg'=>'请输入正确的18位身份证号','code'=>12];
    }
    ...
    public static function getInfoByCard($card_id,$verifyFormat=false){
        if($verifyFormat){
            $verifyFormat=self::verifyFormat($card_id);
            if(!$verifyFormat['status']){
                return $verifyFormat;
            }
        }
     ...
     * 将身份信息发送到联通接口进行验证
     */
    public static function sendVerifyApiByUnicom($name,$card_id){
        if(!$name) {
            return ['status'=>false,'msg'=>'姓名不能为空','verify'=>false,'query'=>false];//qurey指是否提交已提交到过接口查询，
        }
        $card_id=strval($card_id);
        if(!self::verifyFormat($card_id)['status']) {
            return ['status'=>false,'msg'=>'证件格式不合法','verify'=>false,'query'=>false];
        }
        $card_m=self::findOne(['card'=>$card_id]);
        if($card_m && $card_m['name']==$name && $card_m['status']==1){
            return ['status'=>true,'msg'=>'验证通过','verify'=>true,'query'=>true];
        }
        ...
        $data="CertAdress=&bizType=normal&cityCode=591&goodsId=591403121719&idCardVal={$card_id}&opeSysType=2&provinceCode=59&psptTypeCode=02&receiverNameVal={$name}&tmplId=60000019";
        $err_code=\common\tool\http::http_str_post($url,$data);//发送请求
        $err_code=json_decode($err_code,true);//将json结果转换成数组
        if(!isset($err_info[$err_code])){
            return ['status'=>false,'msg'=>'查询接口异常','verify'=>false,'query'=>true];
        }
        if($err_info[$err_code]=='正确'){
            ...
            }elseif(empty($card_m['name'])){
                $card_m->name=$name;
                $card_m->edit_time=time();
                $card_m->status=1;
                if(!$card_m->save()){
                    //记录日志
                    print_r($card_m->getFirstErrors());
            ...
            return ['status'=>true,'msg'=>$err_info[$err_code],'verify'=>true,'query'=>true];
        }else{
            return ['status'=>true,'msg'=>$err_info[$err_code],'verify'=>false,'query'=>true];
        }
        return ['status'=>false,'msg'=>'查询异常','verify'=>false,'query'=>true];
    }
}