 ...
 * 例子：
 * $str="52152519910906049";
 * $id=new verifyIdentify($str);
 * $code=$id->verifyFormat();
 * echo $id->getCodeName($code);
 *
 * 输出
 * 请输入18位身份证号
 *
 ...
 * @property array $api_error_code
 * @property array $format_error
 * @property array $last_check_code
 * @property array $area
 * @property array $area_all
 */
class verifyIdentify
{
    ...
    public function verifyFormat()
    {
        ...
        //地区检验
        if(self::getArea(substr($this->identify, 0, 6))==15){
            return 15;
        }
        switch(strlen($this->identify)){
            /*case 15:
            ...
            case 18:
                ...
                if ((int)( substr($this->identify,6, 4)) % 4 == 0 || ((int)( substr($this->identify,6, 4)) % 100 == 0 && (int)( substr($this->identify,6, 4)) % 4 == 0)) {
                    $ereg ='/^[1-9][0-9]{5}(19|20)[0-9]{2}((01|03|05|07|08|10|12)(0[1-9]|[1-2][0-9]|3[0-1])|(04|06|09|11)(0[1-9]|[1-2][0-9]|30)|02(0[1-9]|[1-2][0-9]))[0-9]{3}[0-9Xx]$/';//闰年出生日期的合法性正则表达式
                } else {
                    $ereg ='/^[1-9][0-9]{5}(19|20)[0-9]{2}((01|03|05|07|08|10|12)(0[1-9]|[1-2][0-9]|3[0-1])|(04|06|09|11)(0[1-9]|[1-2][0-9]|30)|02(0[1-9]|1[0-9]|2[0-8]))[0-9]{3}[0-9Xx]$/';//平年出生日期的合法性正则表达式
                }
                if (preg_match($ereg,$this->identify)) {  //测试出生日期的合法性
                    //计算校验位
                    $S=((int)$this->identify[0]+(int)$this->identify[10])*7
                        +((int)$this->identify[1]+(int)$this->identify[11])*9
                        +((int)$this->identify[2]+(int)$this->identify[12])*10
                        +((int)$this->identify[3]+(int)$this->identify[13])*5
                        +((int)$this->identify[4]+(int)$this->identify[14])*8
                        +((int)$this->identify[5]+(int)$this->identify[15])*4
                        +((int)$this->identify[6]+(int)$this->identify[16])*2
                        +(int)$this->identify[7]*1
                        +(int)$this->identify[8]*6
                        +(int)$this->identify[9]*3;
                    $Y = $S % 11;
                    $M = "F";
                    $JYM = "10X98765432";
                    $M = $JYM . substr($Y, 1);//判断校验位
                    if($this->identify['17']=='x'||$this->identify['17']=='X'){
                        $check='X';
                    }else{
                        $check=$this->identify['17'];
                    }

                    if($this->last_check_code[$Y]==$check){
                        return 10;
                    }else{
                        return 14;
                    }
    ...
    /**
     * 获取地区信息，如果获取不到返回错误码15
     * @param $area
     * @return bool|string
     */
    public  function getArea($area){
        if(isset($this->area_all[$area])){
            return $this->area_all[$area];
        }
        return 15;
    }
     ...
     * @return bool|string
     */
    public  function getAreaByCard($IDCard){
        $area = (int)substr($IDCard,0,2);
        $area_all= (int)substr($IDCard,0,6);
        $result['province']=isset($this->area[$area])?$this->area[$area]:null;
        $result['area']=isset($this->area_all[$area_all])?$this->area_all[$area_all]:null;
        return $result;
    }
    ...
    public  function getCodeName($code){
        if(isset($this->format_error[$code])){
            return $this->format_error[$code];
        }elseif(isset($this->api_error_code[$code])){
            return $this->api_error_code[$code];
        }else{
            return false;
        }
    }
...
//    /**
//     * 将格式正确的身份证信息发送到api进行验证
//     */
//    public function verifyApi(){
//        $format=self::verifyFormat();
//        if($format==10){
//
//        }else{
//            return $format;
